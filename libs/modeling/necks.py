...
class FPN1D(nn.Module):
    ...
    def __init__(
        ...
        self.mask_feat_conv = MaskedConv1D(out_channel, out_channel, 1, bias=False) 
    ...
    def forward(self, inputs, fpn_masks):
        ...
        mask_feat, _ = self.mask_feat_conv(fpn_feats[0], fpn_masks[0])
        return  mask_feat, fpn_feats, new_fpn_masks
...
class FPNIdentity(nn.Module):
    def __init__(
        ...
        for i in range(self.start_level, self.end_level):
            ...
            # layer norm for order (B C T)
            if with_ln:
                fpn_norm = InsNorm1d(out_channel, affine=True) if with_ins_norm else LayerNorm(out_channel)
            else:
                fpn_norm = nn.Identity()
            self.fpn_norms.append(fpn_norm)
