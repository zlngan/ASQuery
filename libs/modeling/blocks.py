...
class MaskedConv1D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # compute the mask
        if self.stride > 1:
            # downsample the mask using nearest neighbor  使用最近临法对mask进行下采样
...
def get_sinusoid_encoding(n_position, d_hid):
    ...
    inv_freq = 1 / (10000 ** (torch.arange(0.0, d_hid, 2.0) / d_hid))
    position = torch.arange(n_position)
    sinusoid_inp = torch.einsum('i,j->ij', position, inv_freq)
    sinusoid_table = torch.stack((sinusoid_inp.sin(), sinusoid_inp.cos()), dim=-1).flatten(-2)
...
class MaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        self.query_norm = InsNorm1d(n_embd, affine=True) if with_ins_norm else LayerNorm(self.n_embd)
        ...
        self.key_norm = InsNorm1d(n_embd, affine=True) if with_ins_norm else LayerNorm(self.n_embd)
        self.value_conv = MaskedConv1D(
            self.n_embd, self.n_embd, kernel_size,
            stride=stride, padding=padding, groups=self.n_embd, bias=False
        )
        self.value_norm = InsNorm1d(n_embd, affine=True) if with_ins_norm else LayerNorm(self.n_embd)
...
class LocalMaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        self.query_conv = MaskedConv1D(
            self.n_embd, self.n_embd, kernel_size,
            stride=stride, padding=padding, groups=self.n_embd, dilation=conv_dila, bias=False
        )
        self.query_norm = InsNorm1d(n_embd, affine=True) if with_ins_norm else LayerNorm(self.n_embd)
    ...
    def _pad_and_transpose_last_two_dims(x, padding):
        ...
        x = nn.functional.pad(x, padding) #[8,255,18,18] ->[8,255,19,18]
        x = x.view(*x.size()[:-2], x.size(-1), x.size(-2)) # [8,255,18,19]
        return x

    @staticmethod
    def _mask_invalid_locations(input_tensor, affected_seq_len):
        beginning_mask_2d = input_tensor.new_ones(affected_seq_len, affected_seq_len + 1).tril().flip(dims=[0]) #[9,10] 左上角是1，其他是0
        beginning_mask = beginning_mask_2d[None, :, None, :] #[1,9,1,10]
        ending_mask = beginning_mask.flip(dims=(1, 3)) # [1,9,1,10] 右下角是1，其他是0
        beginning_input = input_tensor[:, :affected_seq_len, :, : affected_seq_len + 1] #[2,9,4,10]
        beginning_mask = beginning_mask.expand(beginning_input.size()) # [2,9,4,10]
    ...
    def _sliding_chunks_query_key_matmul(
        ...
        # B * num_heads, head_dim, #chunks=(T//w - 1), 2w # [8, 2304, 128]
        chunk_query = self._chunk(query, window_overlap) # [8,255,18,128]
        chunk_key = self._chunk(key, window_overlap) #  [8,255,18,128]

        # matrix multiplication
        # bcxd: batch_size * num_heads x chunks x 2window_overlap x head_dim
        # bcyd: batch_size * num_heads x chunks x 2window_overlap x head_dim
        # bcxy: batch_size * num_heads x chunks x 2window_overlap x 2window_overlap
        diagonal_chunked_attention_scores = torch.einsum( # [8,255,18,18]
            "bcxd,bcyd->bcxy", (chunk_query, chunk_key))

        # convert diagonals into columns
        # B * num_heads, #chunks, 2w, 2w+1
        diagonal_chunked_attention_scores = self._pad_and_transpose_last_two_dims( #[8,255,18,19]
        ...
        diagonal_attention_scores = diagonal_chunked_attention_scores.new_empty( #[8,256,9,19]
        ...
        diagonal_attention_scores = diagonal_attention_scores.view( #[8,256,9,19] ->[2,2304,4,19]
    ...
    def forward(self, x, mask):
        ...
        # step 1: depth convolutions 类似于第一步的conv，但是使用了depth-wise方式，用这种方式对输入进行投影
        # query conv -> (B, nh * hs, T')
        q, qx_mask = self.query_conv(x, mask)
        q = self.query_norm(q)
        # key, value conv -> (B, nh * hs, T'')
        ...
        att = self._sliding_chunks_query_key_matmul( #[2,2304,4,19]
        ...
        if self.use_rel_pe:
            att += self.rel_pe
        # kv_mask -> B, T'', 1
        inverse_kv_mask = torch.logical_not( # [2,1,2304] ->[2,2304,1]
        ...
        diagonal_mask = self._sliding_chunks_query_key_matmul( #[2,2304,1,19] #超过边界的位置用-inf，超过动作的区域用-10000，其余地方为0
        ...
        att = self.attn_drop(att) # 一般是对attn进行dropout

        # step 4: compute attention value product + output projection
        # chunked attn value product -> B, nh, T, hs
        out = self._sliding_chunks_matmul_attn_probs_value( # [2,4,2304,128]
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        with_ins_norm=False    # if to use instance norm for input
    ):
        super().__init__()
        assert len(n_ds_strides) == 2
        # layer norm for order (B C T)
        self.ln1 =  InsNorm1d(n_embd, affine=True) if with_ins_norm else LayerNorm(n_embd)
        self.ln2 = InsNorm1d(n_embd, affine=True) if with_ins_norm else LayerNorm(n_embd)
    ...
    def forward(self, x, mask, pos_embd=None):
        ...
        out, out_mask = self.attn(self.ln1(x), mask) # 进行了一次注意力
        out_mask_float = out_mask.to(out.dtype)
        out = self.pool_skip(x) * out_mask_float + self.drop_path_attn(out) # skip-connection
        # FFN
        out = out + self.drop_path_mlp(self.mlp(self.ln2(out)) * out_mask_float) # 经过两层mlp
...
class ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        dilation=1,            # dilation rate for conv
    ):
        super().__init__()
        ...
        self.conv1 = MaskedConv1D(
            n_embd, width, kernel_size, n_ds_stride, padding=padding, dilation=dilation)
        self.conv2 = MaskedConv1D(
            width, n_out, kernel_size, 1, padding=padding, dilation=dilation)

        # attach downsampling conv op
        if n_ds_stride > 1:
            # 1x1 strided conv (same as resnet)
            self.downsample = MaskedConv1D(n_embd, n_out, 1, n_ds_stride, dilation=dilation)
        else:
...
class MaskedMHCCA(MaskedMHCA):
    ...
    def forward(self, query, key, value, mask):
        B_q, C_q, T_q = query.size()
        B_f, C_f, T_f = key.size()
        # query conv -> (B, nh * hs, T')
        q = self.query_conv(query)
        q = self.query_norm(q)
        # key, value conv -> (B, nh * hs, T'')
        k, kv_mask = self.key_conv(key, mask)
        k = self.key_norm(k)
        ...
        # move head forward to be the batch dim
        # (B, nh * hs, T'/T'') -> (B, nh, T'/T'', hs)
        q = q.view(B_q, self.n_head, self.n_channels, -1).transpose(2, 3)
        k = k.view(B_f, self.n_head, self.n_channels, -1).transpose(2, 3)
        v = v.view(B_f, self.n_head, self.n_channels, -1).transpose(2, 3)
        ...
        out = att @ (v * kv_mask[:, :, :, None].to(v.dtype))
        # re-assemble all head outputs side by side
        out = out.transpose(2, 3).contiguous().view(B_q, C_q, -1)
...
class TransformerdecoderLayer(nn.Module): # 得到一个layer，里面包含多个blocks
    def __init__(
        self,
        n_embd,                # dimension of the input features
        n_head,               # number of attention heads
        ...
        with_feat_pos=False       # if to add rel position encoding to attention
    ):
        super().__init__()
    ...
    def forward(self, x, feats, masks, query_pos, feat_pos):
        assert len(feats) == self.n_blocks
        action_queries = []
        for i, block in enumerate(self.decoder_layer):
            # x = block(x, feats[i], masks[i])
            x = block(x, feats[len(feats)-i-1], masks[len(feats)-i-1], query_pos, feat_pos[len(feats)-i-1]) # from low to high 
            action_queries.append(x)
        return x, action_queries


class TransformerDecoderBlock(nn.Module): # 得到一个block，里面包含cross attn， self attn和ffn
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
    def __init__(
        ...
        with_query_pos=False,
        with_feat_pos=False,
        with_ins_norm=False,       # if to add rel position encoding to attention
    ):
        super().__init__()
        assert len(n_ds_strides) == 2
        ...
        self.ln5 = LayerNorm(n_embd)


        self.cross_attn = MaskedMHCCA(
            n_embd=n_embd,
            n_head=n_head,
            n_qx_stride=n_ds_strides[0],
            n_kv_stride=n_ds_strides[1],
    ...
    def forward(self, x, feat, mask, query_pos, feat_pos):
        x = x + query_pos if self.with_query_pos is True else x
        key = feat + feat_pos.to(feat.device)*mask.to(feat.device).detach() if self.with_feat_pos is True else feat
        value = feat
        # pre-LN transformer: https://arxiv.org/pdf/2002.04745.pdf
        out, _ = self.cross_attn(self.ln1(x), self.ln2(key), self.ln3(value), mask) # cross-attn
        out = self.pool_skip(x) + self.drop_path_attn(out) # skip-connection
        out = self.self_attn(self.ln4(out)) # self-attn
        out = self.pool_skip(x) + self.drop_path_attn(out) # skip-connection
        # FFN
        out = out + self.drop_path_mlp(self.mlp(self.ln5(out))) # 经过两层mlp
        # optionally add pos_embd to the output
        # if pos_embd is not None:
        #     out += pos_embd * out_mask_float
        return out
...
class MHCA(MaskedMHCA):
    def __init__(
        ...
        super().__init__(
            n_embd=n_embd,          # dimension of the output features
            n_head=n_head,          # number of heads in multi-head self-attention
            n_qx_stride=n_qx_stride,   # dowsampling stride for query and input
            n_kv_stride=n_kv_stride,   # downsampling stride for key and value
            attn_pdrop=attn_pdrop,  # dropout rate for the attention map
            proj_pdrop=proj_pdrop,  # dropout rate for projection op
            with_ins_norm=with_ins_norm,
        )
...
class TransformerdecoderLayer(nn.Module): # 得到一个layer，里面包含多个blocks
    ...
    def forward(self, x, feats, masks, query_pos, feat_pos, mask_func, boundary_func):
        assert len(feats) == self.n_blocks
        action_queries = []
        for i, block in enumerate(self.decoder_layer):
            # x = block(x, feats[i], masks[i])
