...
def make_optimizer(model, optimizer_config):
    ...
    decay = set()
    no_decay = set()
    whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv1d, MaskedConv1D, torch.nn.Embedding, torch.nn.MultiheadAttention)
    blacklist_weight_modules = (LayerNorm, torch.nn.GroupNorm, torch.nn.InstanceNorm1d, torch.nn.LayerNorm)
    # blacklist_weight_modules = (torch.nn.GroupNorm, torch.nn.InstanceNorm1d)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith('bias'):
                ...
                no_decay.add(fpn)
            # elif pn.endswith('query_pe.weight'):
            #     decay.add(fpn)
            # elif pn.endswith('query.weight'):
            #     decay.add(fpn)
            # elif pn.endswith('level_embd.weight'):
            #     decay.add(fpn)
...
def train_one_epoch(
    ...
    # main training loop
    print("\n[Train]: Epoch {:d} started".format(curr_epoch+1)) # epoch starts from 1
    import time
    start = time.time()
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        # flops, params = profile(model.module, (video_list,))
        ...
        if (iter_idx != 0) and (iter_idx % print_freq) == 0:
            ...
            block1 = 'Epoch: [{:03d}][{:05d}/{:05d}]'.format(
                curr_epoch+1, iter_idx, num_iters
            )
...
def eval_one_epoch(
    ...
    # mapping_dict = {}
    # with open('data/breakfast/mapping.txt', 'r') as f:
    #     line = f.readlines()
    #     for l in line:
    #         mapping_dict[l.split(' ')[0]] = l.split(' ')[1].strip()
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            ...
            output = model(video_list)
            ### save results
            # video_name = video_list[0]['video_id']
            # with open(os.path.join('results_boundary', video_name + '.txt'), 'a+') as f:
            #     true_boundary = output[0][-1][-1]
            #     for j in true_boundary:
            #         f.write(str(j[-1]) + ' ')

            # pred_boundary = output[0][-1][0].cpu().numpy()
            # np.save(os.path.join('results_boundary', video_name + '.npy'), pred_boundary)

            # with open(os.path.join('results_refine_boundary', video_name + '.txt'), 'a+') as f:
            #     pred_no_boundary = output[0][1]
            #     for j in pred_no_boundary:
            #         f.write(mapping_dict[str(j.item())] + '\n')

            # unpack the results into ANet format
            num_vids = len(output)
            for vid_idx in range(num_vids):
                results.append(output[vid_idx])
    ...
    # log mAP to tb_writer
    if tb_writer is not None:
        tb_writer.add_scalar('val/Acc', eval_results['Acc'], curr_epoch+1)
        tb_writer.add_scalar('val/Edit', eval_results['Edit'], curr_epoch+1)
        tb_writer.add_scalar('val/F1_0.1', eval_results['F1_0.1'], curr_epoch+1)
        tb_writer.add_scalar('val/F1_0.25', eval_results['F1_0.25'], curr_epoch+1)
        tb_writer.add_scalar('val/F1_0.5', eval_results['F1_0.5'], curr_epoch+1)
