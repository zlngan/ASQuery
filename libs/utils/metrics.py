...
class SegEval(object):
    ...
    def evaluate(self, results):
        ...
        nums = len(gts)
        tp, fp, fn = np.zeros(len(self.overlaps)), np.zeros(len(self.overlaps)), np.zeros(len(self.overlaps))
        correct = 0
        total = 0
        edit = 0
        eps=1e-6
        ...
        for i in tqdm(range(nums)):
            gt = gts[i].to(self.device)
            pred = preds[i].to(self.device)
            for j in range(len(gt)):
                total += 1
                if gt[j] == pred[j]:
                    correct += 1
            edit += self.edit_score(pred, gt)
            for s in range(len(self.overlaps)):
                tp1, fp1, fn1 = self.f_score(pred, gt, self.overlaps[s])
                tp[s] += tp1
                fp[s] += fp1
                fn[s] += fn1
        ...
        for s in range(len(self.overlaps)):
            ...
            f1 = 2.0 * (precision*recall) / (precision+recall) if precision+recall >0 else eps
            f1 = np.nan_to_num(f1)*100
            eval_results[f'F1_{self.overlaps[s]}'] = f1
            # print ('F1@%0.2f: %.4f' %(self.overlap[s], f1))

        eval_results['Acc'] = acc
        eval_results['Edit'] = edit
